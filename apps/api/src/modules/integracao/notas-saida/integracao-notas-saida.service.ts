...
export class IntegracaoNotasSaidaService {
  ...
  async upsert(
    ...
    apiKeyId: string,
    input: IntegracaoNotaSaidaCreate,
  ): Promise<{ registro: IntegracaoNotaSaida; decisao: DecisaoUpsert }> {
  ...
  upsertLote(
    empresaId: string,
    apiKeyId: string,
    registros: IntegracaoNotaSaidaLoteItem[],
    ...
    return processarLote(registros, async (item) => {
      ...
      const { decisao } = await this.upsert(
        empresaId,
        apiKeyId,
        item as IntegracaoNotaSaidaCreate,
      );
