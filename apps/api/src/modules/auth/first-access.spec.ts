...
import { completeFirstAccessSchema } from '@plataforma/contracts';
...
describe('Primeiro acesso', () => {
  ...
  const prisma = { withTenant: jest.fn() };
  ...
  it('não regrava dados na repetição da confirmação', async () => {
    tx.usuario.findUniqueOrThrow.mockResolvedValue({ primeiroAcessoConcluidoEm: new Date() });
    await service.completeFirstAccess('usuario', 'empresa', input);
