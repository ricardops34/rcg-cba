...
/**
 * O cliente Prisma que o gerador usa — cliente inteiro **ou** transação.
 *
 * É `Prisma.TransactionClient` de propósito, e não `PrismaClient`: o tipo mais
 * estreito é o que os dois chamadores têm em comum, e é o que impede alguém de
 * abrir uma transação aninhada aqui dentro (o Postgres não tem, e o Prisma
 * recusa).
 */
export type DemoDb = Prisma.TransactionClient;

/** O que a operação produziu — o CLI imprime, a tela mostra. */
export interface ResumoDemo {
  empresa: string;
  /** Os meses povoados, em MM/AAAA. */
  periodo: string[];
  usuarios: number;
  /** O administrador ganhou cadastro de vendedor (ver o comentário abaixo). */
  adminVirouVendedor: boolean;
  clientes: number;
  produtos: number;
  notas: number;
  titulos: number;
  titulosComBoleto: number;
  orcamentos: number;
  conversas: number;
  atividades: number;
  /** A senha dos usuários criados — a mesma para todos, é demonstração. */
  senha: string;
  acessos: { email: string; perfil: string }[];
}
...
/**
 * Os meses que a demonstração cobre: **seis inteiros antes do corrente**, o
 * corrente e o seguinte. `-6` a `+1` a partir de hoje.
 *
 ...
 *
 * Eram quatro meses (`-2` a `+1`) até 2026-09-19. Seis meses fechados é o que
 * dá base a uma comparação: as Consultas de venda aceitam 12 meses, e um
 * gráfico com três pontos não mostra tendência nenhuma numa demonstração.
 * O custo é proporcional — mais notas, mais títulos e mais XML por rodada.
 */
const MESES = [-6, -5, -4, -3, -2, -1, 0, 1].map((deslocamento) => {
...
const RAMOS = [
  {
    nome: 'Restaurantes',
    cnae: '5611201',
    prefixos: ['Restaurante', 'Cantina', 'Bistrô'],
  },
  { nome: 'Bares', cnae: '5611204', prefixos: ['Bar', 'Petiscaria', 'Adega'] },
  {
    nome: 'Padarias',
    cnae: '4721102',
    prefixos: ['Padaria', 'Confeitaria', 'Panificadora'],
  },
  {
    nome: 'Mercados',
    cnae: '4711302',
    prefixos: ['Mercado', 'Supermercado', 'Mercearia'],
  },
  { nome: 'Hotéis', cnae: '5510801', prefixos: ['Hotel', 'Pousada', 'Motel'] },
  {
    nome: 'Lanchonetes',
    cnae: '5611203',
    prefixos: ['Lanchonete', 'Espetinho', 'Food Truck'],
  },
];
...
/**
 * Gera o conjunto de demonstração na empresa indicada.
 *
 * Apaga o conjunto anterior (`apagarDemo`) antes de criar o novo — é o que
 * torna a operação repetível sem acumular lixo. Nada fora do prefixo
 * `DEMO-` é tocado.
 *
 * `db` é o cliente Prisma **ou** a transação de `withTenant`. Pelo script
 * ele chega como cliente da role dona, sem RLS; pela API, como transação já
 * com o tenant definido — e aí é a própria RLS que impede o gerador de
 * alcançar outra empresa, em vez de um `where` que alguém pode esquecer.
 */
export async function gerarDemo(
  db: DemoDb,
  empresaId: string,
): Promise<ResumoDemo> {
  ...
  for (const [i, descricao] of [
    'Armazém Central',
    'Armazém Filial',
  ].entries()) {
    armazens.push(
      await db.armazem.create({
        data: {
          empresaId,
          codigoErp: `${PREFIXO}A${i + 1}`,
          descricao,
          ativo: true,
        },
      }),
  ...
  for (const [i, [descricao, forma]] of CONDICOES.entries()) {
    condicoes.push(
      await db.condicaoPagamento.create({
        data: {
          empresaId,
  ...
  const tabelaPreco = await db.tabelaPreco.create({
    data: {
      empresaId,
  ...
  const equipe = [
    ...
    {
      nome: 'Sérgio Almeida',
      chave: 'supervisor1.demo',
      tipo: 'superior' as const,
      perfil: 'Supervisor',
    },
    {
      nome: 'Regina Matos',
      chave: 'supervisor2.demo',
      tipo: 'superior' as const,
      perfil: 'Supervisor',
    },
    {
      nome: 'Paulo Vieira',
      chave: 'vendedor1.demo',
      tipo: 'vendedor' as const,
      perfil: 'Vendedor',
    },
    {
      nome: 'Carla Nunes',
      chave: 'vendedor2.demo',
      tipo: 'vendedor' as const,
      perfil: 'Vendedor',
    },
    {
      nome: 'Diego Ramos',
      chave: 'vendedor3.demo',
      tipo: 'vendedor' as const,
      perfil: 'Vendedor',
    },
    {
      nome: 'Bianca Lopes',
      chave: 'vendedor4.demo',
      tipo: 'vendedor' as const,
      perfil: 'Vendedor',
    },
    {
      nome: 'Rafael Torres',
      chave: 'vendedor5.demo',
      tipo: 'vendedor' as const,
      perfil: 'Vendedor',
    },
    {
      nome: 'Juliana Peixoto',
      chave: 'vendedor6.demo',
      tipo: 'vendedor' as const,
      perfil: 'Vendedor',
    },
  ];
   ...
   */
  const vinculoAdmin = await db.usuarioEmpresa.findFirst({
    where: {
      empresaId,
      ativo: true,
  ...
  if (admin) {
    const vendedorAdmin = await db.vendedor.create({
      data: {
        empresaId,
        usuarioId: admin.id,
  ...
  const supervisores = [
    pessoas['supervisor1.demo'],
    pessoas['supervisor2.demo'],
  ];
  ...
  // ---- clientes, por ramo ---------------------------------------------------
  const cnaes = await db.cnae.findMany({
  ...
  const cestaDoRamo = RAMOS.map((_, indiceRamo) => {
    const nucleo = produtos.filter(
      (_, i) => i % RAMOS.length !== indiceRamo % RAMOS.length,
    );
    ...
    for (const m of MESES) {
      ...
      for (let n = 0; n < entre(1, 2); n++) {
        ...
        const linhas = escolhidos.slice(0, entre(3, 7)).map((produto) => {
          ...
          const unitario =
            Math.round(produto.ultimoPreco * (1 - entre(0, 10) / 100) * 100) /
            100;
          return {
            produto,
            quantidade: qtd,
        ...
        const valor =
          Math.round(linhas.reduce((s, l) => s + l.vlrTotal, 0) * 100) / 100;
          ...
          data: {
            ...
            itens: {
              create: linhas.map((l, item) => ({
                ...
                ...(l.devolvido
                  ? {
                      quantidadeDev: Math.max(
                        Math.round(l.quantidade * 0.2),
                        1,
                      ),
        ...
        const conteudoXml = montarXmlNfe({
          ...
          destinatario: {
            ...
            codigoMunicipio:
              MUNICIPIOS.find((mu) => mu[0] === cliente.municipio)?.[2] ??
              '5002704',
            uf: cliente.uf,
            cep: cliente.cep,
            inscricaoEstadual: String(entre(100000000, 999999999)),
        ...
        for (const [p, dup] of duplicatas.entries()) {
          ...
          const titulo = await db.tituloReceber.create({
            data: {
              empresaId,
  ...
  for (const [i, cliente] of clientes.entries()) {
    ...
    for (const item of itens) {
      item.vlrTotal =
        Math.round(item.quantidade * item.vlrUnitario * 100) / 100;
    }
    const total =
      Math.round(itens.reduce((s, it) => s + it.vlrTotal, 0) * 100) / 100;
    ...
    const orcamento = await db.orcamento.create({
      data: {
        ...
        dataRetorno:
          status === 'enviado' ? new Date(criadoEm.getTime() + 7 * DIA) : null,
        vlrTotal: total,
  ...
  await db.orcamentoConfig.upsert({
    where: { empresaId },
  ...
  for (const pessoa of todasAsPessoas.filter((p) => p.tipo === 'vendedor')) {
    for (const m of MESES) {
      const faturado =
        faturadoPorVendedorMes.get(`${pessoa.vendedorId}-${m.ano}-${m.mes}`) ??
        0;
      ...
      const objetivo = await db.objetivoVendedorMes.create({
        data: {
          empresaId,
      ...
      for (const [i, categoria] of categorias.entries()) {
        ...
        restante -= valor;
        await db.objetivoVendedorCategoria.create({
          data: {
            empresaId,
  ...
  const comAparelho = [
    ...vendedores,
    ...supervisores,
    ...(pessoas['admin'] ? [pessoas['admin']] : []),
  ];
  const sessoes = new Map<string, string>();
  for (const pessoa of comAparelho) {
    const sessao = await db.whatsappSessao.create({
      data: {
        empresaId,
        vendedorId: pessoa.vendedorId,
  ...
  for (const [indice, cliente] of clientesComConversa.entries()) {
    ...
    const contato = await db.whatsappContato.create({
      data: {
        empresaId,
        jid: `55679${entre(10000000, 99999999)}@s.whatsapp.net`,
    ...
    const conversa = await db.whatsappConversa.create({
      data: {
        empresaId,
        sessaoId,
    ...
    await db.whatsappConversa.update({
  ...
  for (const registro of conversaPorClienteDia.values()) {
    atividades.push({
      ...
      descricao: [
        ...
        registro.entrada
          ? plural(registro.entrada, 'recebida', 'recebidas')
          : null,
      ]
        .filter(Boolean)
        .join(' · '),
  ...
  return {
    ...
    periodo: MESES.map((m) => `${String(m.mes).padStart(2, '0')}/${m.ano}`),
    usuarios: equipe.length,
    adminVirouVendedor: !!admin,
    clientes: clientes.length,
    produtos: produtos.length,
    notas: notas.length,
    titulos: titulos.length,
    titulosComBoleto: titulos.filter((t) => t.comBoleto).length,
    orcamentos: orcamentos.length,
    conversas: clientesComConversa.length,
    atividades: atividades.length,
    senha: SENHA_DEMO,
    acessos: equipe.map((p) => ({
      email: `${p.chave}@bjsoft.com.br`,
      perfil: p.perfil,
    })),
  };
}

export async function apagarDemo(db: DemoDb, empresaId: string) {
  ...
  const conversas = await db.whatsappConversa.findMany({
    where: { empresaId, sessao: { vendedorId: { in: vendedorIds } } },
    select: { id: true, contatoId: true },
  });
  const conversaIds = conversas.map((c) => c.id);
  await db.whatsappMensagemAgendada.deleteMany({
    where: { conversaId: { in: conversaIds } },
  });
  await db.whatsappAcaoRegistro.deleteMany({
    where: { conversaId: { in: conversaIds } },
  });
  await db.whatsappMensagem.deleteMany({
    where: { conversaId: { in: conversaIds } },
  });
  await db.whatsappConversa.deleteMany({ where: { id: { in: conversaIds } } });
  await db.whatsappContato.deleteMany({
    where: { id: { in: conversas.map((c) => c.contatoId) } },
  });
  await db.whatsappSessao.deleteMany({
  ...
  await db.oportunidade.deleteMany({
    where: { empresaId, vendedorId: { in: vendedorIds } },
  });
  await db.objetivoVendedorCategoria.deleteMany({
    where: { objetivo: { vendedorId: { in: vendedorIds } } },
  });
  await db.objetivoVendedorMes.deleteMany({
    where: { empresaId, vendedorId: { in: vendedorIds } },
  });
  await db.comunicado.deleteMany({
  ...
  await db.tituloReceber.deleteMany({
    where: { empresaId, codigoErp: { startsWith: PREFIXO } },
  });
  ...
  await db.clienteCnae.deleteMany({
  ...
  await db.condicaoPagamento.deleteMany({
    where: { empresaId, codigoErp: { startsWith: PREFIXO } },
  });
  await db.armazem.deleteMany({
    where: { empresaId, codigoErp: { startsWith: PREFIXO } },
  });
  await db.categoria.deleteMany({
    where: { empresaId, codigoErp: { startsWith: PREFIXO } },
  });
  await db.vendedor.deleteMany({ where: { id: { in: vendedorIds } } });
}
